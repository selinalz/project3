...
function showSkies() {
  ...
  skies.forEach(sky => {
    ...
    var skyLocation = document.createElement("div");
    skyLocation.classList.add("location");
    skyLocation.innerText = sky.fields.location;
    dateContainer.append(skyLocation);

    skyContainer.addEventListener("click", function() {

      document.querySelectorAll(".location").forEach(location => location.style.display = "none");
      skyLocation.style.display = "block";

      if (skyContainer.classList.contains("50")) {
        skyLocation.style.color = "black";
      } else {
        skyLocation.style.color = "white";
      }
    });
